...
import {
  DEFAULT_CHECKPOINT,
  DEFAULT_GLEAGUE_LINK_CACHE,
  DEFAULT_LOG,
  DEFAULT_NBA_BREF_LINK_CACHE,
...
function printUsage(): void {
  console.log(`G-League-Scraper — Basketball Reference G League → Hoop Central season stats

Usage:
  npm run scrape -- [options]

Options:
  --backfill             Crawl G League A–Z index and ingest all players
  ...
  --player-slug <slug>   Single player test (e.g. curryse01d)
...
function parseArgs(argv: string[]): ScrapeOptions & { showHelp: boolean } {
  ...
  return {
    ...
    logPath: DEFAULT_LOG,
    gleagueLinkCachePath: DEFAULT_GLEAGUE_LINK_CACHE,
    nbaBrefLinkCachePath: DEFAULT_NBA_BREF_LINK_CACHE,
...
async function main(): Promise<void> {
  ...
  console.log("Starting G-League-Scraper");
