...
import type { GLeaguePlayerMeta } from "./types.js";

const USER_AGENT =
  "Mozilla/5.0 (compatible; HoopCentralGLeagueScraper/1.0; +https://github.com/hoopcentral)";
...
export class BrefClient {
  ...
  playerUrl(slug: string): string {
    ...
    return `https://www.basketball-reference.com/gleague/players/${letter}/${slug}.html`;
  }

  indexUrl(letter: string): string {
    return `https://www.basketball-reference.com/gleague/players/${letter.toLowerCase()}/`;
  }

  teamUrl(abbrev: string, seasonYear: number): string {
    return `https://www.basketball-reference.com/gleague/teams/${abbrev.toUpperCase()}/${seasonYear}.html`;
  }
  ...
  async listSlugsForLetter(letter: string): Promise<string[]> {
    ...
    for (const match of html.matchAll(/href="\/gleague\/players\/[a-z]\/([a-z0-9]+)\.html"/gi)) {
  ...
  parsePlayerMeta(slug: string, html: string): GLeaguePlayerMeta {
...
export function parsePlayerMetaFromHtml(slug: string, html: string): GLeaguePlayerMeta {
  const titleMatch = /<title>([^<]+)<\/title>/i.exec(html);
  let displayName = slug;
  if (titleMatch) {
    const title = titleMatch[1].replace(/\s+/g, " ").trim();
    const nameMatch = /^(.+?)\s+G-League\s+Stats/i.exec(title);
    if (nameMatch) displayName = nameMatch[1].trim();
  }

  const birthMatch = /id="necro-birth"[^>]*data-birth="(\d{4}-\d{2}-\d{2})"/i.exec(html);
  const birthDate = birthMatch?.[1] ?? null;

  return { slug, displayName, birthDate };
}
...
export function parseTeamNameFromTitleHtml(html: string): string {
  ...
  const title = titleMatch[1].replace(/\s+/g, " ").trim();
  const match = /^\d{4}-\d{2}\s+(.+?)\s+Stats/i.exec(title);
  if (!match) {
